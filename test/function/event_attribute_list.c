#include <stdlib.h>
#include <stdio.h>
#include <string.h>

#include "private/event_attribute_list.h"
#include "private/static/event_attribute_list.h"
#include "private/type.h"

#include "helper.h"

const char * test_appender( void );
const char * test_begin( void );
const char * test_constructor( void );
const char * test_copy( void );
const char * test_destructor( void );
const char * test_is_empty( void );
const char * test_next( void );
const char * test_prepender( void );

int
main( void )
{
  unsigned failure_count = 0;
  const char * result;
  
  RUN_TEST( appender )
  RUN_TEST( begin )
  RUN_TEST( constructor )
  RUN_TEST( copy )
  RUN_TEST( destructor )
  RUN_TEST( is_empty )
  RUN_TEST( next )
  RUN_TEST( prepender )
  
  if( failure_count > 0 )
    return EXIT_FAILURE;
  else
    return EXIT_SUCCESS;
}

const char *
test_appender
( void )
{
  Status * status;
  EventAttributeList * list = NewEventAttributeList();
  if( list == NULL )
    return "the list was not created";
  
  EventAttribute * attribute_1 = BuildEventAttribute();
  attribute_1->name = "first test attribute";
  
  EventAttribute * attribute_2 = BuildEventAttribute();
  attribute_2->name = "second test attribute";
  
  EventAttribute * attribute_3 = BuildEventAttribute();
  attribute_3->name = "third test attribute";
  
  EventAttribute * attribute_4 = BuildEventAttribute();
  attribute_4->name = "fourth test attribute";
  
  status = AppendToEventAttributeList( list, attribute_1 );
  if( status != NULL )
    return "the node was not successfully added";
  
  status = AppendToEventAttributeList( list, attribute_2 );
  if( status != NULL )
    return "the node was not successfully added";
  
  status = AppendToEventAttributeList( list, attribute_3 );
  if( status != NULL )
    return "the node was not successfully added";
  
  status = AppendToEventAttributeList( list, attribute_4 );
  if( status != NULL )
    return "the node was not successfully added";
  
  EventAttribute * attribute = BeginEventAttributeList( list );
  FAIL_IF_NULL( attribute, "the list did not have a first node" )
  if( attribute != attribute_1 )
    return "the first attribute was not correct";
  
  return NULL;
}

const char *
test_begin
( void )
{
  EventAttributeList * list = BuildEventAttributeList();
  FAIL_IF_NULL( list, "could not build the test list" )
  
  EventAttribute * attribute = BeginEventAttributeList( list );
  FAIL_IF_NULL( attribute, "a attribute was returned from a newly-constructed list" )
  
  return NULL;
}

const char *
test_constructor
( void )
{
  EventAttributeList * list = NULL;
  
  list = NewEventAttributeList();
  
  if( list == NULL )
    return "the list was not created";
  
  EventAttribute * attribute = BeginEventAttributeList( list );
  FAIL_IF_NOT_NULL( attribute, "a newly-constructed list already had members" )
  
  return NULL;
}

const char *
test_copy
( void )
{
  EventAttributeList * list = BuildEventAttributeList();
  if( list == NULL )
    return "could not build the test list";
  
  EventAttributeList * copy;
  
  copy = CopyEventAttributeList( NULL );
  if( copy != NULL )
    return "the copy was not null for a null pointer";
  
  copy = CopyEventAttributeList( list );
  FAIL_IF_NULL( copy, "the copy was null for a non-null pointer" )
  if( copy == list )
    return "the copy was equal to the original list";
  
  EventAttribute * original_attribute = BeginEventAttributeList( list );
  EventAttribute * copy_attribute = BeginEventAttributeList( copy );
  while( original_attribute != NULL ){
    if( original_attribute != copy_attribute )
      return "the copy was not an accurate copy of the original";
    original_attribute = NextInEventAttributeList( list );
    copy_attribute = NextInEventAttributeList( copy );
  }
  
  return NULL;
}

const char *
test_destructor
( void )
{
  EventAttributeList * list = NewEventAttributeList();
  
  if( list == NULL )
    return "the list was not created";
  
  DestroyEventAttributeList( list );
  
  list = BuildEventAttributeList();
  if( list == NULL )
     return "could not build the test list";
  
  DestroyEventAttributeList( list );
  
  return NULL;
}

const char *
test_is_empty
( void )
{
  EventAttributeList * list = NULL;
  if( !EventAttributeListIsEmpty( list ) )
    return "a null list pointer was deemed empty";
  
  list = NewEventAttributeList();
  if( !EventAttributeListIsEmpty( list ) )
    return "a newly created list pointer was not deemed empty";
  
  list = BuildEventAttributeList();
  if( list == NULL )
    return "could not build the test list";
  if( EventAttributeListIsEmpty( list ) )
    return "a full list was deemed empty";
  
  return NULL;
}

const char *
test_next
( void )
{
  EventAttributeList * list = BuildEventAttributeList();
  FAIL_IF_NULL( list, "could not build the test list" )
  
  EventAttribute * attribute = BeginEventAttributeList( list );
  FAIL_IF_NULL( attribute, "a attribute was not returned from the start call" )
  
  attribute = NextInEventAttributeList( list );
  FAIL_IF_NULL( attribute, "a attribute was not returned from the next call" )
  
  return NULL;
}

const char *
test_prepender( void )
{
  EventAttributeList * list = NewEventAttributeList();
  FAIL_IF_NULL( list, "could not build a new test list" )
  
  EventAttribute * attribute = BuildEventAttribute();
  FAIL_IF_NULL( list, "could not build the test attribute" );
  
  Status * status = PrependToEventAttributeList( NULL, NULL );
  FAIL_IF_NULL( status, "two empty arguments did not generate an abnormal status" )
  ASSERT_STRINGS_EQUAL( "empty argument", status->name, "two empty arguments did not generate the appropriate status code" )
  
  status = PrependToEventAttributeList( list, NULL );
  FAIL_IF_NULL( status, "an empty value did not generate an abnormal status" )
  ASSERT_STRINGS_EQUAL( "empty argument", status->name, "an empty value did not generate the appropriate status code" )
  
  status = PrependToEventAttributeList( NULL, attribute );
  FAIL_IF_NULL( status, "a NULL list did not generate an abnormal status" )
  ASSERT_STRINGS_EQUAL( "empty argument", status->name, "a NULL list did not generate the appropriate status code" )
 
  status = PrependToEventAttributeList( list, attribute );
  FAIL_IF_NOT_NULL( status, "a value could not be prepended to an empty list" )
  EventAttribute * retrieved_attribute = BeginEventAttributeList( list );
  FAIL_IF_NULL( retrieved_attribute, "the list's nodes were invalid" )
  if( retrieved_attribute != attribute )
    return "the attribute was not actually prepended to the list";
  
  list = BuildEventAttributeList();
  FAIL_IF_NULL( list, "could not build a populated test list" ) 
  attribute = BuildEventAttribute();
  FAIL_IF_NULL( list, "could not build a test attribute" )
  status = PrependToEventAttributeList( list, attribute );
  FAIL_IF_NOT_NULL( status, "the attribute was not correctly prepended to a populated list" )
  retrieved_attribute = BeginEventAttributeList( list );
  FAIL_IF_NULL( retrieved_attribute, "a populated list had its members removed" )
  if( retrieved_attribute != attribute )
    return "the attribute was not actually prepended to a full list";
  
  return NULL;
}
