...
( void )
{
  ...
  if( CompareStrings( str_0, str_0, options ) != 0 )
    return "the same value was not equal";
  
  if( CompareStrings( str_0, str_1, options ) != 0 )
    return "two equal strings were not equal";
  
  if( CompareStrings( str_1, str_2, options ) >= 0 )
    return "first did not come before second";
  
  if( CompareStrings( str_2, str_1, options ) <= 0 )
    return "second did not come after first";
  
  if( CompareStrings( str_3, str_4, options ) >= 0 )
