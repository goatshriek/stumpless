...
/** @file
 * Macro functions that log messages and entries at the critical level.
 *
 * These can be turned into no-ops at compile time by defining
 * STUMPLESS_DISABLE_CRIT_LEVEL during build, or at least before inclusion of
 * this header (or stumpless.h).
 */

#ifndef __STUMPLESS_LEVEL_CRIT_H
#  define __STUMPLESS_LEVEL_CRIT_H

#  include <stumpless/level/mask.h>

#  ifndef STUMPLESS_DISABLE_CRIT_LEVEL
...
/**
 * Logs a message to the current target with critical severity.
 *
 * This function will be removed at compile time if
 * STUMPLESS_DISABLE_CRIT_LEVEL has been defined during build. If it is
 ...
 * a build with critical level calls disabled.
 *
 * This function will log the given message with a severity of
 * STUMPLESS_SEVERITY_CRIT, and the facility defined by the
 * STUMPLESS_DEFAULT_FACILITY. If you wish to specify a different priority, then
 * you will need to use stumplog_c instead.
 *
...
/**
 * Adds an entry to a given target with critical severity.
 *
 * This function will be removed at compile time if
 * STUMPLESS_DISABLE_CRIT_LEVEL has been defined during build. If it is
 * disabled, then this function is removed at compile time and will have no
 ...
 */
#  ifdef STUMPLESS_DISABLE_CRIT_LEVEL
#    define stump_c_entry( target, entry ) ( 0 )
#  else
#    define stump_c_entry( target, entry )                                     \
 ...
 */
#  ifdef STUMPLESS_DISABLE_CRIT_LEVEL
#    define stump_c_log( target, priority, ... ) ( 0 )
#  else
#    define stump_c_log( target, priority, ... )                               \
...
/**
 * Adds a message to a given target with critical severity.
 *
 * This function will be removed at compile time if
 * STUMPLESS_DISABLE_CRIT_LEVEL has been defined during build. If it is
 * disabled, then this function is removed at compile time and will have no
 ...
 * STUMPLESS_SEVERITY_CRIT, and the facility defined by the
 * STUMPLESS_DEFAULT_FACILITY. If you wish to specify a different priority, then
 * you will need to use stump_c_log instead.
 *
 * The message must be a valid format specifier string provided along with the
 * appropriate number of variable arguments afterwards. This means that it
 * should not be a user-controlled value under any circumstances. If you need a
 * safer alternative without the risks of format strings, use
 * \c stump_c_message_str instead.
 *
 * **Thread Safety: MT-Safe**
 * This function is thread safe. Different target types handle thread safety
 * differently, as some require per-target locks and others can rely on system
 ...
 */
#  ifdef STUMPLESS_DISABLE_CRIT_LEVEL
#    define stump_c_message( target, ... ) ( 0 )
#  else
#    define stump_c_message( target, ... )                                     \
 ...
 */
#  ifdef STUMPLESS_DISABLE_CRIT_LEVEL
#    define stumplog_c( priority, ... ) ( ( void ) 0 )
#  else
#    define stumplog_c( priority, ... ) stumplog( ( priority ), __VA_ARGS__ )
#  endif
