...
( Tree *tree, void *value )
{
  ...
  dimensions = BeginList( tree->dimensions );
...
( Tree *tree )
{
  ...
  copy = malloc( sizeof( Tree ) );
  if( !copy )
    return NULL;

  dimension_list = NewList();
  if( !dimension_list )
    return NULL;
...
( void )
{
  ...
  dimension = malloc( sizeof( Dimension ) );
  if( !dimension )
    return NULL;
...
( Node *node )
{
  ...
  left = node->left_children[0];
  right = node->right_children[0];
...
( Iterator *iterator )
{
  ...
  current = PeekAtStack( iterator->path );
  value = current->value;
...
{
  int difference;
  Node *current, *first, *middle, *last, *tree_1, *tree_2, *tree_3, *tree_4, *great_grandparent, *grandparent, *parent, *bottom;
  Stack *changed;
  unsigned index, left_height, right_height;
  ...
  grandparent = PopFromStack( changed );
  parent = PopFromStack( changed );
  bottom = PopFromStack( changed );
  ...
  great_grandparent = PeekAtStack( stack );
